...
const ZigDoc = lsp_server.ZigDoc;
...
const InnerError = error{ OutOfMemory, AnalysisFail, ChangePending };
...
pub fn generate(gpa: Allocator, zig_doc: *ZigDoc) (Allocator.Error || error{ChangePending})!Zir {
    const tree = zig_doc.tree;
    ...
    if (zig_doc.getChangePending()) return error.ChangePending;
    ...
    const fatal = if (tree.errors.len == 0) fatal: {
        if (AstGen.structDeclInner(
            ...
            error.OutOfMemory, error.ChangePending => |e| return e,
...
fn containerMember(
    ...
    if (gz.astgen.zig_doc.getChangePending()) return error.ChangePending;
    const astgen = gz.astgen;
    const tree = astgen.tree;
...
fn scanContainer(
    ...
    for (members) |member_node| {
        if (astgen.zig_doc.getChangePending()) return error.ChangePending;
