...
pub fn generate(gpa: Allocator, tree: Ast, change_pending: *std.atomic.Value(bool)) Allocator.Error!Zir {
    ...
    if (change_pending.load(.acquire) == true) {
        // std.log.info("!AstCheck : early exit", .{});
...
fn structDeclInner(
    ...
    for (container_decl.ast.members) |member_node| {
        if (astgen.change_pending.load(.acquire) == true) {
            // std.log.info("!AstCheck : loop exit", .{});
            return error.AnalysisFail;
        }
