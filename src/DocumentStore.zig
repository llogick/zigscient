    ...
    fn setSource(
        ...
        const custom_ast: CustomAst = custom_ast: {
            const reusable_data: CustomAst.ReusableData = reuse: {
                ...
                if (!(text_idx_hi < max_text_idx)) text_idx_hi = max_text_idx;
                // Always retokenize whole line(s), because .comment(s)
                ...
                //     self.tree.source[text_idx_lo..text_idx_hi],
                // });
                ...
                const null_node_state: CustomAst.State = .{
                    .nodes_len = 0,
                    .xdata_len = 0,
                    .token_ind = 0,
                };
                ...
                // std.log.debug("root decls len: {any}", .{self.tree.rootDecls().len});
                ...
                for (root_decls, 0..) |root_decl, idx| {
                    ...
                    const prev_node_state = if (tok_i == 0 or prev_node_idx == 0) null_node_state else self.tree_nstates.get(root_decls[prev_node_idx]) orelse break;
                    ...
                    if (prev_node_state.token_ind != 0) for (root_decls[0 .. prev_node_idx + 1]) |value| scratch.appendAssumeCapacity(value);
                    const scratch_len = scratch.items.len;
                    var new_nstates: CustomAst.States = .empty;
                    errdefer new_nstates.deinit(gpa);
                    if (prev_node_state.token_ind != 0) for (self.tree_nstates.keys()) |key| {
                    ...
                    reuse_upper_nodes: for (root_decls[idx + 1 ..], idx + 1..) |root_decl2, idx2| {
                        ...
                        break :custom_ast CustomAst.parse(
                            self.impl.allocator,
                            ...
                            error.OvershotCutOff => {
                                ...
                                new_nstates.clearRetainingCapacity();
