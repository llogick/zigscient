...
pub fn build(b: *Build) !void {
    ...
    { // zig build release
        const targets = comptime if (proj_version.pre == null) release_targets ++ additional_tagged_release_targets else release_targets;
        ...
        release(b, &release_artifacts, resolved_proj_version);
    }
...
fn release(b: *Build, release_artifacts: []const *Build.Step.Compile, released_proj_version: std.SemanticVersion) void {
    ...
    if (released_proj_version.pre != null and released_proj_version.build == null) {
    ...
    const source = b.fmt(
        ...
        \\
    , .{
        .proj_version = released_proj_version,
